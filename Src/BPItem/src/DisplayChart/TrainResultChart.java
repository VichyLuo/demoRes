...
public class TrainResultChart {
	public ChartPanel frame1;
	public TrainResultChart(BPNetual bp) {
		XYDataset xydataset = createDataSet(bp);
		JFreeChart jfreechart = ChartFactory.createXYLineChart("训练数据与网络计算结果比较图", "样本次数", "输出值", xydataset,PlotOrientation.VERTICAL, true,true,true);
	...
	public XYDataset createDataSet(BPNetual bp) {
		...
		XYSeries xyseries = new XYSeries("训练数据网络计算输出值");
		int count = 1;
		for (int i=0; i<bp.caseNum; i++) {
			for (int j=0; j<bp.outNum; j++) {
				xyseries.add(count, bp.out_out[i][j]);
				++count;
			}
		}
		XYSeries xyseries1 = new XYSeries("训练数据期望输出值");
		count = 1;
		for (int i=0; i<bp.caseNum; i++) {
			for (int j=0; j<bp.outNum; j++) {
				xyseries1.add(count, bp.output[i][j]);
				++count;
			}
		}
